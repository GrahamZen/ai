...
一致代价搜索在广度优先搜索上做了改进，扩展节点时优先选择代价最低的节点。即每次在边界集合中选出用于扩展的节点，标记该节点已访问，并遍历每个可行的动作，找出未访问的节点加入边界集合，访问过的节点如果代价更小，则更新边界集合。
...
def UCS(maze,start,end):
    ...
    frontier = []
    route=[]
    visited = [[0] * len(maze[0]) for _ in range(len(maze))] # 0表示没有访问，1表示已经访问过了
...
为了说明启发式函数对实验的影响，我也使用了欧氏距离的平方进行了实验。
...
<center class="half">
    <img src="17338233_zhenggehan_lab6.assets\Manhattan.svg" width="400"/>
    <img src="17338233_zhenggehan_lab6.assets\Chebyshev.svg" width="400"/>
</center>

<center class="half">
    <img src="17338233_zhenggehan_lab6.assets\Euler.svg" width="400"/>
    <img src="17338233_zhenggehan_lab6.assets\Quadratic.svg" width="400"/>
</center>
...
相比一致代价搜索，A*搜索的时间花费明显减少，许多UCS访问的位置都没有访问。空间复杂度则基本没有区别。使用三种不同的启发式函数也没有很大的差别。从数值上来看，曼哈顿距离的时间最短。可以注意到，平方距离得到的结果不是最优的，启发式函数需要一致性和可采纳性才能保证最优性。曼哈顿距离在满足两个特性的前提下数值更大，可能是搜索效率较高的原因之一。
...
| 策略 | 优点|缺点 |
| ------------|------|------|
| 一致代价搜索  | 容易实现 | 空间复杂度较高，盲目搜索所以效率低 |
| A*搜索        | 搜索速度较快 | 搜索速度取决于启发式函数的特点，而且有时候难以找到满足两个性质的启发式函数 |
| 迭代加深搜索  | 空间复杂度低 | 迭代重新搜索时已访问的点仍会被重新访问。开销不一致时需要满足一定条件才有最优性 |
| IDA*          | 加入启发式函数，搜索效率提高 | 和迭代加深搜索一样 |
